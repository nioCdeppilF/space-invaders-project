...
class Integrate:
    ...
    def draw(self, canvas):
        ...
        self.update()
        for i in range(55): # made it 55 as thats how many aliens are initially in the list decreasing it decreases the speed
            if self.clock.transition(ALIEN_SPEED):
                self.update_alien_movement()  # update alien movement to follow the tick
        # The range changing as you eliminate aliens was causing the biggest bug we encountered
    ...
    def update_alien_movement(self):  # change name
        ...
        for a in range(len(self.alien_list)):
            ...
            if self.alien_list[a].pos.get_p()[0] <= 15:
                ...
                self.vel = Vector(0.5, 0)
            if self.alien_list[a].pos.get_p()[0] > CW - 15:
                self.drop(self.vel.x)
                self.vel = Vector(-0.5, 0)
    ...
    def game_over_text(self, canvas):
        ...
        if self.game_over:
            ...
            if self.game_won:
                ...
                end_timer = threading.Timer(5, end_game) #Final screen display time
                end_timer.start()
            else:
...
# Create a frame
frame = simplegui.create_frame("Space Invaders v10", CW, CH)
clock = Clock()
